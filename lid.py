    ...
    with open(path, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            if first:
                first = False
                low0 = raw_key.lower()
                header_keywords = ("錄音檔檔名", "辨認出之客語漢字", "filename", "text", "檔名", "客語漢字")
...
def main():
    ...
    print("完成")
    print(f" - {OUT_DAPU_LIST}（大埔腔清單）")
    print(f" - {OUT_ZHAOAN_LIST}（詔安腔清單）")
    print(f" - {OUT_MIX_LIST}平手簡單檔名清單）")
    print(f" - {OUT_TIE}（平手詳細區塊，最上方含總結；分為「不同錯字」與「相同錯字」兩節）")
    if missing_wav:
